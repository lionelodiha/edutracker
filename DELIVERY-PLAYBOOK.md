...
| **4 · Portfolio track** | 10 modules | Can I show an employer I can do this? | 3–4 months part-time |

Parts 1–3 are what EduTracker **needs**, in order. Each is the cheapest thing that
makes the next worth doing — a pipeline with no tests just ships bugs faster, and
a deploy you cannot observe is one you cannot safely repeat.

**Part 4 is a different goal.** It covers Linux, AWS, Kubernetes, Terraform,
Ansible and the monitoring stack — the things job adverts ask for, which this
workload does not justify. It is deliberately over-engineered, and the framing at
the top of it explains how to talk about that without sounding like
cargo-culting. It needs Part 1 only, so it can run alongside Parts 2 and 3.

The one ordering exception inside Parts 1–3: the **health endpoints** in Stage 06
are two hours with no dependencies, and Part 2 needs them. Pull those forward.
...
**This list is about production, not about learning.** Part 4 builds every one of
these deliberately, for a different reason. The two are not in conflict — see the
framing at the top of it.

---

# PART 4 — THE PORTFOLIO TRACK

**Ten modules · 3–4 months part-time · needs Part 1 only**

Parts 1–3 build what EduTracker *needs*. This part builds what a DevOps job
advert *asks for*. Those are different lists, and pretending otherwise helps
nobody.

## Read this framing first, it is the whole point

Part 3 tells you not to run Kubernetes. Part 4 has you run Kubernetes. That is
deliberate, and how you talk about it decides whether it reads as competence or
as cargo-culting.

The wrong version, in an interview: *"I used Kubernetes, Terraform and ArgoCD."*
Every bootcamp graduate says this sentence.

The right version: *"EduTracker runs fine on one container service — the load is
a few hundred users at one school and Kubernetes would have been a system needing
its own on-call for no reliability gain. I built the Kubernetes version anyway so
I'd know the trade-off rather than guess it. Here's what it cost, and here's the
NAT Gateway bill that taught me to read a pricing page."*

The second answer demonstrates judgement, which is the thing that is actually
scarce. **You cannot credibly say you chose not to use a tool until you can use
it.** That is what this part is for.

Keep both deployments. The comparison *is* the portfolio.

## Before you start: the money

This is the part that catches people. A learning account can quietly cost more
than a phone contract.

| Service | Real cost | Note |
| --- | --- | --- |
| **NAT Gateway** | ~$32/month + data | The classic bill shock. Runs 24/7 whether used or not. |
| **EKS control plane** | ~$73/month | Charged per cluster, always on, no free tier. |
| **ALB** | ~$16/month + LCU | Per load balancer. |
| **RDS `db.t3.micro`** | free 12 months | 750 h/month, then ~$15/month. |
| **EC2 `t3.micro`** | free 12 months | 750 h/month. |
| **ECS Fargate** | pay per task-second | Much cheaper than EKS for this size. |

Four rules that will save you real money:

1. **Learn Kubernetes locally first.** `kind` or `k3d` on your laptop is free and
   behaves like the real thing for everything in Module 5. Only move to EKS when
   you specifically want EKS on the CV.
2. **Set a billing alarm before you create anything.** AWS Budgets, $10 threshold,
   email alert. Do this on day one.
3. **`terraform destroy` every single night.** This is why Module 4 comes before
   Module 5 — once the infrastructure is code, tearing it down and rebuilding it
   in the morning costs one command. That habit is also good practice.
4. **Avoid NAT Gateway while learning.** Put workloads in public subnets with
   tight security groups, or use VPC endpoints. Production wants NAT; your
   learning account does not.

---

## Module 1 · Linux

The foundation everything else sits on, and the thing most self-taught people
skip. Interviewers probe it because it separates people who understand systems
from people who memorised tool names.

**Learn:** the filesystem hierarchy, permissions and ownership (including setuid
and umask), processes and signals, systemd units and journald, package managers,
SSH and key management, networking (`ip`, `ss`, `dig`, `curl -v`, `tcpdump`
basics), disk and memory inspection (`df`, `du`, `free`, `top`/`htop`), log
hunting (`grep`, `awk`, `sed`, `journalctl -u`).

**Build against EduTracker:** provision a bare Ubuntu VM. Install .NET, Postgres
and nginx by hand. Run the API as a **systemd unit** with proper restart policy,
a dedicated non-login user, and logs going to journald. Put nginx in front as a
reverse proxy with TLS from Let's Encrypt.

**Done when:** the VM reboots and EduTracker comes back up with no manual step.

**Prepares you for:** *"A service is running but not responding. Walk me through
diagnosing it."* — the single most common DevOps interview question.

---

## Module 2 · Docker, deeper

You already have working Dockerfiles. This is the difference between working and
good.

**Learn:** layer caching and ordering, build context and `.dockerignore`, image
size reduction (multi-stage, distroless, alpine trade-offs), multi-arch builds
with buildx, `HEALTHCHECK`, running as non-root, read-only root filesystems,
registries and tagging strategy, `docker system prune` and disk reclamation.

**Build:** get the API image as small as you reasonably can, measure before and
after, and write down what each change saved. Build for `amd64` and `arm64`. Add
a `HEALTHCHECK` pointing at `/health/ready`.

**Done when:** you can explain every line of your Dockerfile and why it is in
that order.

**Prepares you for:** *"Your image is 1.2GB. How do you make it smaller?"*

---

## Module 3 · AWS fundamentals

Do this by hand in the console **once**, then never again — Module 4 replaces it
with code. Clicking through it first is what makes the Terraform meaningful.

**Learn:** VPC, subnets (public vs private), route tables, internet and NAT
gateways, security groups vs NACLs, IAM (users, roles, policies, and why roles
beat access keys), EC2, RDS, S3, ALB, Route53, CloudWatch logs and metrics,
Secrets Manager or Parameter Store.

**Build:** deploy EduTracker into a VPC you designed. API on EC2 or ECS, Postgres
on RDS in a private subnet, ALB in front with an ACM certificate, secrets in
Parameter Store, logs in CloudWatch.

**Done when:** it is reachable over HTTPS on a real domain, and the database is
**not** publicly addressable — verify that by trying to connect from your laptop
and failing.

**Prepares you for:** *"How does traffic get from a user's browser to a container
in a private subnet?"* Be able to draw it.

---

## Module 4 · Terraform

The single most valuable tool on this list for employability.

**Learn:** providers and resources, state and **remote state with locking**
(S3 + DynamoDB), variables and outputs, modules, `for_each` vs `count`, data
sources, `plan` vs `apply`, workspaces vs directory-per-environment, drift, and
importing existing resources.

**Build:** every piece of Module 3, as code. Split into modules — `network`,
`database`, `compute`, `dns`. Two environments, staging and production, from the
same modules with different variables. Remote state in S3.

**Done when:** `terraform destroy` then `terraform apply` rebuilds the whole
environment and the app works, with no console clicks.

**Prepares you for:** *"Someone changed a security group in the console. What
happens on your next apply, and how do you handle it?"*

> **Do this module before Module 5.** Once teardown and rebuild are one command
> each, you stop leaving expensive clusters running overnight out of fear of
> losing your setup.
...
Learn it locally on `kind` first. Only move to EKS at the end.

**Learn:** pods, deployments, replicasets, services (ClusterIP / NodePort /
LoadBalancer), ingress, ConfigMaps and Secrets, **liveness, readiness and startup
probes**, resource requests and limits, HPA, namespaces, RBAC, StatefulSets and
PVCs, rolling updates and rollbacks, `kubectl describe` and `logs` for debugging,
CrashLoopBackOff and how to read it.

**Build:** manifests for the whole stack. Map the probes directly onto the health
endpoints from Stage 06 — `/health/live` for liveness, `/health/ready` for
readiness. Migrations as a Kubernetes `Job` that must complete before the
deployment rolls, exactly as Stage 04 describes. Then do it once on EKS with
IRSA for AWS permissions.

**Done when:** a rolling update happens with no dropped requests, and you can
prove it with a load generator running during the deploy.

**Prepares you for:** *"What's the difference between a liveness and a readiness
probe?"* — and Stage 06 already taught you the answer that most candidates get
wrong.
...
**Learn:** chart structure, templating, `values.yaml` and overrides per
environment, `helm upgrade --install`, rollbacks, hooks, dependencies,
`helm template` for debugging.

**Build:** convert Module 5's manifests into one chart with `values-staging.yaml`
and `values-production.yaml`.

**Done when:** one command deploys either environment.

---

## Module 7 · Prometheus, Grafana and Loki

Stage 06 gave you the theory. This builds the stack.

**Learn:** Prometheus scrape config and service discovery, **PromQL** (this is
the part worth real practice), recording and alerting rules, Alertmanager routing
and silences, Grafana dashboards and variables, Loki and LogQL, and the
exporters — node, postgres, blackbox.

**Build:** scrape the OpenTelemetry metrics from Stage 06. Build three
dashboards: service health, database, and business (organizations, active
sessions). Then implement the **burn-rate alerts** from Stage 06 as real
Prometheus rules — multi-window, 14.4× over 1 h paging and 6× over 6 h ticketing.

**Done when:** you deliberately break something and the right alert fires with
the right severity.

**Prepares you for:** *"Write a PromQL query for the 95th percentile latency of
the checkout endpoint over the last hour."* Practise until this is quick.

---

## Module 8 · GitOps with ArgoCD

**Learn:** the pull vs push deployment model, why cluster credentials in CI is an
anti-pattern, Application and ApplicationSet resources, sync policies and
self-healing, drift detection, app-of-apps, and secret handling with Sealed
Secrets or External Secrets Operator.

**Build:** a separate config repo. ArgoCD watches it and syncs. CI's only job
becomes: build image, push, update the tag in the config repo. Your pipeline
stops holding cluster credentials entirely.

**Done when:** you change a replica count in git, touch nothing else, and the
cluster converges.
...
Less fashionable than the rest, still all over job adverts — especially anywhere
with on-premise or hybrid infrastructure.

**Learn:** inventory, playbooks, roles, idempotency, variables and templating
with Jinja2, handlers, Ansible Vault.

**Build:** a playbook that does Module 1 automatically — bare Ubuntu to a running,
TLS-terminated EduTracker. Run it against two hosts.

**Done when:** running it twice changes nothing the second time. That is
idempotency, and it is the concept being tested.

---

## Module 10 · Scripting

**Learn:** Bash (strict mode `set -euo pipefail`, trap, arguments, exit codes,
`jq`) and Python (boto3, requests, argparse, virtualenvs).

**Build:** a backup script with retention and verification. A Python tool using
boto3 that reports untagged or idle AWS resources and their monthly cost. A
pre-commit hook that blocks secrets.

**Done when:** the backup script is on a schedule and you have restored from it
once.
...
Sequential, because each builds on the last. Roughly 3–4 months at evenings and
weekends.

| Weeks | Modules | Why here |
| --- | --- | --- |
| 1–2 | 1 Linux | Everything else assumes it |
| 3 | 2 Docker | Small step from what you have |
| 4–5 | 3 AWS by hand | Makes Terraform meaningful |
| 6–8 | 4 Terraform | Now teardown is free, so the rest is cheap |
| 9–12 | 5 Kubernetes | Local first, EKS briefly at the end |
| 13 | 6 Helm | Small once K8s is understood |
| 14–16 | 7 Prometheus, Grafana | Stage 06 theory becomes real |
| 17–18 | 8 ArgoCD | Needs a working cluster |
| 19–20 | 9 Ansible, 10 Scripting | Can run in parallel |

## What to show an employer

Not a list of tools. Three things:

1. **The repository**, with both deployments living side by side and a README
   explaining why each exists.
2. **An architecture diagram** of the AWS footprint you can talk through from
   memory.
3. **One incident write-up.** Break something on purpose, debug it properly,
   write the blameless postmortem from Stage 08. A single good postmortem
   demonstrates more than ten green pipelines, because almost no junior candidate
   has one.

> **On certifications.** AWS Solutions Architect Associate and CKA are the two
> that reliably get a CV past a filter. Neither substitutes for the repository —
> take them *while* building, so the exam material has something real to attach
> to, and so you have something to talk about when the interview moves past the
> certificate.
