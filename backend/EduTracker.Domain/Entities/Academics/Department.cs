...
public sealed class Department : IEntity, IAuditable
{
    public AuditState AuditState { get; private set; } = new();

    private Department() { }

    public Department(Guid organizationId, Guid? facultyId, string name, string? description)
    {
    ...
    // Lets a school adopt faculties later without recreating departments.
    ...
    private static string ValidateName(string name)
    {
        ...
        if (normalizedLength < AcademicLimits.DepartmentNameMinLength || normalizedLength > AcademicLimits.DepartmentNameMaxLength)
            throw new ArgumentException(
                $"Department name must be between {AcademicLimits.DepartmentNameMinLength} and {AcademicLimits.DepartmentNameMaxLength} characters.",
                nameof(name)
            );
    ...
    private static string? ValidateDescription(string? description)
    {
        ...
        if (normalized.Length > AcademicLimits.DepartmentDescriptionMaxLength)
            throw new ArgumentException(
                $"Department description must not exceed {AcademicLimits.DepartmentDescriptionMaxLength} characters.",
                nameof(description)
            );
