...
// Validation rules mirror RegisterUserCommandValidator so the rules stay consistent.
internal sealed class AddStaffMemberCommandValidator : AbstractValidator<AddStaffMemberCommand>
{
    public AddStaffMemberCommandValidator()
    {
        ...
        RuleFor(x => x.Role)
            .Must(r => r is OrganizationMemberRole.Admin
                        or OrganizationMemberRole.Teacher
                        or OrganizationMemberRole.Student
                        or OrganizationMemberRole.Moderator
                        or OrganizationMemberRole.Member)
            .WithMessage("Role must be Admin, Teacher, Student, Moderator, or Member. Owners cannot be provisioned directly.");
    }
