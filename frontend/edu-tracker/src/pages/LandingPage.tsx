        ...
        <div className="lp-preview">
            ...
            <div key={tab} className="lp-preview-main" id="preview-panel" role="region" aria-labelledby={`preview-tab-${previewTabs.indexOf(tab)}`}>
                ...
                <div className="lp-preview-table"><div className="lp-table-heading"><h4>{tab === "Members" ? "Your team" : tab === "Academic year" ? "Academic structure" : "Your courses"}</h4><span>{tab === "Members" ? "MEMBERS & ROLES" : "AUTUMN · 2026"}</span></div>
                    <div className="lp-table-columns"><span>{tab === "Members" ? "Name" : tab === "Academic year" ? "Semester / term" : "Course name"}</span><span>{tab === "Members" ? "Role" : "Details"}</span><span>Status</span></div>
                    {(tab === "Members" ? [ ["AO", "Alex Ojo", "Organization owner", "Owner", "Joined"], ["SW", "Sarah Williams", "Teaching team", "Teacher", "Joined"], ["JD", "James Davis", "Teaching team", "Teacher", "Invited"] ] : tab === "Academic year" ? [["01", "First semester", "2026 / 2027", "Autumn term", "Active"], ["02", "Second semester", "2026 / 2027", "Spring term", "Upcoming"], ["03", "Third semester", "2026 / 2027", "Summer term", "Upcoming"]] : [["MA", "Further Algebra", "MATH-201", "8 classes", "Active"], ["EN", "English Literature", "ENG-102", "6 classes", "Active"], ["PH", "Applied Physics", "PHY-201", "10 classes", "Active"]]).map((row, index) => <div className="lp-table-row" key={row[0]}><div><span className={`lp-course-icon lp-tone-${index}`}>{row[0]}</span><span><strong>{row[1]}</strong><small>{row[2]}</small></span></div><span>{row[3]}</span><span className={`lp-row-status ${row[4] === "Upcoming" || row[4] === "Invited" ? "is-pending" : ""}`}>{row[4]}</span></div>)}
                </div>
            </div>
        </div>
    ...
    return <div className="lp" ref={rootRef}>
        ...
        <main id="main">
            ...
            <section className="lp-features lp-shell" id="product" aria-labelledby="features-title"><div className="lp-section-heading"><div><span className="lp-overline">LESS BUSYWORK. MORE BIG PICTURE.</span><h2 id="features-title">Everything connected.<br /><span>Nothing lost in the shuffle.</span></h2></div><p>Give your academic year a home. Keep the structure clear, your team connected, and the details within reach.</p></div>
                <div className="lp-feature-grid"><article className="lp-feature lp-feature-wide"><span className="lp-feature-symbol"><Icon name="book" size={24} /></span><h3>A place for every part of the year.</h3><p>Connect semesters, terms, courses, and classes in one clear structure. See how every piece fits together.</p><div className="lp-structure" aria-label="Academic hierarchy"><div><span>01</span> Ridgeway Academy <small>Organization</small></div><div><span>02</span> 2026 / 2027 <small>Semester</small></div><div><span>03</span> Autumn <small>Term</small></div><div><span>04</span> Further Algebra <small>Course</small></div><div><span>05</span> Monday · Period 4 <small>Class</small></div></div></article>
            ...
            <section className="lp-workflow" id="workflow" aria-labelledby="workflow-title"><div className="lp-shell"><div className="lp-centered-heading"><span className="lp-overline">A FRESH START, WITHOUT THE FRICTION</span><h2 id="workflow-title">From your first sign-in<br />to your next school year.</h2></div><div className="lp-steps">{[{ title: "Make yourself at home", text: "Create your account. Your dashboard is the starting point for everything ahead." }, { title: "Bring your school together", text: "Add your organization, invite your people, and give everyone the right role." }, { title: "Give your year some structure", text: "Set up semesters, add courses, and organize the classes that bring it all to life." }].map((step, index) => <article key={step.title}><span className="lp-step-number">0{index + 1}</span><h3>{step.title}</h3><p>{step.text}</p></article>)}</div></div></section>
